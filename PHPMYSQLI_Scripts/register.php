...
error_reporting(E_ALL);
	if(isset($_POST["email"]) && isset($_POST["username"]) && isset($_POST["password1"]) && isset($_POST["password2"])){
		...
		$mysqli = new mysqli("$db_host","$db_user","$db_pass","$db_name");
			...
			if ($stmt = $mysqli->prepare("SELECT username, email FROM sc_users WHERE email = ? OR username = ? LIMIT 1")) {
		...
		if(count($errors) == 0){
			...
			if ($stmt = $mysqli->prepare("INSERT INTO sc_users (username, email, password) VALUES(?, ?, ?)")) {
